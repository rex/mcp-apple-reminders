...
- **Wire-level integration suite (`tests/integration/`, run `./venv/bin/python -m tests.integration.run $(date +%H%M%S)`)** — **165 checks** against the LIVE store via a fresh stdio server; NOT in the unit gate (needs Reminders permission), self-cleans an `MCP-IntegTest` group/list. This is the ONLY layer that catches native-helper crashes + structured-output/datetime wire bugs (unit tests mock the helper). It surfaced **two real bugs**: (1) `set_urgent` crashed the helper (`urgentAlarmContext` selector removed on current macOS, uncaught NSException) — **FIXED v0.1.93** (see the dedicated gotcha below); (2) `create_smart_list` without `filter_data_b64` errors "filterData is required" (contradicts its docs) — still a self-flipping expected-error known-issue with a fix task queued. SQLite read-after-write lag on hashtag/subtask/section rows is real — the suite polls, don't assert once.
