// rem_reminderkit — Objective-C ReminderKit (private framework) helper
//                   subprocess for mcp-apple-reminders.
//
// BORROWED FROM:
//   https://github.com/viticci/remctl
//   File:    remctl-private.m
//   Commit:  baaa57b922b6379989cb2cc04f54461faefb1496 (RemCTL 1.0.3)
//   License: MIT — see _native/THIRD_PARTY_NOTICES.md for the verbatim license text.
//
...
//
// Local modifications:
//   - Added the file-header comment block above (no behavioral change).
//   - Added a `--ping` argv shortcut at the start of main (no behavioral
//     change; emits {"status":"ok","helper":"rem_reminderkit"} and exits 0).
//     Used by verify_setup.py to confirm the binary loads and the
//     ReminderKit private framework symbols resolve without exercising
//     any of the write actions.
//
// WARNING: this helper links against /System/Library/PrivateFrameworks/
// ReminderKit.framework. Private API. May change across macOS releases.
// Pierce explicitly accepted the risk; see spec 002 design.md.
//
// Re-sync from upstream: re-clone the repo at the recorded commit,
// re-apply this header + the --ping shortcut.
...
int main(int argc, const char * argv[]) {
    @autoreleasepool {
        // Patched for mcp-apple-reminders S0.6: lightweight liveness probe.
        // Exits before reading stdin so the helper can be probed from a
        // non-interactive shell without piping a JSON payload.
        if (argc == 2 && strcmp(argv[1], "--ping") == 0) {
            printf("{\"status\":\"ok\",\"helper\":\"rem_reminderkit\"}\n");
            return 0;
        }
