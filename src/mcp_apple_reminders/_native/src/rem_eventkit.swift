// rem_eventkit — Swift EventKit helper subprocess for mcp-apple-reminders.
//
// BORROWED FROM:
//   https://github.com/viticci/remctl
//   File:    remctl-bridge.swift
...
//   - Added a `--ping` CLI argument shortcut at the start of main (no behavioral
//     change; emits {"status":"ok","helper":"rem_eventkit"} and exits 0).
//     Used by verify_setup.py to confirm the binary loads + EventKit symbols
//     resolve without exercising the full JSON dispatch path.
//
...
// Patched for mcp-apple-reminders S0.6: lightweight liveness probe used by
// verify_setup.py. Exits before reading stdin so the helper can be probed
// from a non-interactive shell without piping a JSON payload.
if CommandLine.arguments.count == 2 && CommandLine.arguments[1] == "--ping" {
    print("{\"status\":\"ok\",\"helper\":\"rem_eventkit\"}")
    exit(0)
}
