...
> Branch: `main` (trunk strategy) · Owner (human): @pierce · Last update: 2026-05-29 by Claude (CL-1 cleanup pass)
...
**CL-1 cleanup pass: COMPLETE** (2026-05-29) — all 9 batches (B0–B9) plus the CRITICAL EventKit write-swallow bugfix shipped (commits `aa613fb`→`3b2c2b2`), each gate-green + pushed. Audit + plan live in `docs/audits/2026-05-29-post-spec-002-cleanup-audit/`. One sub-item was DECLINED: `make sync-skeleton` on the 3 hooks (the skeleton v0.37.0 versions regressed `cd … || exit 1`; see §2 + AGENTS §9). **CL-2 capability expansion: IN PROGRESS** — slices 2.1–2.5 shipped (smart lists; list/group appearance + pinning; templates; grocery categorize; urgent / early-reminder / section-create) = **13 new tools, 41→54**. Remaining slices 2.6–2.13: attachments (images, kill-switch-gated), read-side (recently-deleted + filters + ZPARENTREMINDER fix), clear_tags (.m recompile), recurrence/alarm read-back (ADR 0002), ToolAnnotations, typed result models, resources/prompts polish, docs sweep. Plan: `docs/audits/2026-05-29-post-spec-002-cleanup-audit/06-cl2-expansion-plan.md`; tasks #59–66. **Then:** exhaustive integration testing (Pierce's stated next major phase).
