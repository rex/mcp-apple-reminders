...
**CL-1 cleanup pass: COMPLETE** (2026-05-29) — all 9 batches (B0–B9) plus the CRITICAL EventKit write-swallow bugfix shipped (commits `aa613fb`→`3b2c2b2`), each gate-green + pushed. Audit + plan live in `docs/audits/2026-05-29-post-spec-002-cleanup-audit/`. One sub-item was DECLINED: `make sync-skeleton` on the 3 hooks (the skeleton v0.37.0 versions regressed `cd … || exit 1`; see §2 + AGENTS §9). **CL-2 capability expansion: slices 2.1–2.9 SHIPPED** (2026-05-30, v0.1.79) — smart lists; list/group appearance+pinning; templates; grocery; urgent/early-reminder/sections; **2.6 attachments** (generic files unlocked in `rem_reminderkit.m`; local-file tool opt-in behind env `MCP_APPLE_REMINDERS_ENABLE_FILE_ATTACHMENTS`); **2.7 read-side** (`get_recently_deleted` + `reminders://recently-deleted`; `flagged` filter; ZPARENTREMINDER fix → `parent_reminder_id`/`subtasks` populate); **2.8 `clear_tags`** + `reminders://tags`; **2.9 recurrence/alarm/early-reminder read-back** (ADR 0002). Now **59 tools, 7 reminders resources, 2 ADRs**. Plus two wire-level BUG FIXES this session (see AGENTS §9): **datetime structured-output v0.1.77**, **elicitation-guard v0.1.78**. **Remaining CL-2 = polish slices 2.10–2.13**: 2.10 ToolAnnotations on all 59 tools (readOnly/destructive/idempotent/openWorld + human titles); 2.11 typed result models for the ~12 bare-`dict` tools; 2.12 resources/prompts polish (+ `organize_into_sections` prompt + per-param `Field(description=)`); 2.13 regenerate catalog + docs sweep. Plan: `docs/audits/2026-05-29-post-spec-002-cleanup-audit/06-cl2-expansion-plan.md`; tasks #59–66. **USER DIRECTIVE (2026-05-30): autonomously roll through 2.10–2.13 (commit+push per slice), THEN the integration-testing phase — which Pierce specified is FULLY AUTOMATED, agent-driven end-to-end, and EXHAUSTIVE.** Integration fixture already in the store: group `CL29-Alarm-Tests` → list `Alarm Lab` (id `94F21A71-6D6B-47BE-A7D9-329A2E57AD85`), 8 items covering every alarm/recurrence type — KEEP until integration testing is complete.
